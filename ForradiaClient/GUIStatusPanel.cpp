...
void GUIStatusPanel::Render() {
    ...
    string strHP = "HP: " + std::to_string(Global::player->m_hpCurrent) + "/" + std::to_string(Global::player->m_hpMax);
    ...
    int hpBarFilledWidth = barWidth * (double) Global::player->m_hpCurrent / Global::player->m_hpMax;
    ...
    streamNRGYCurrent << std::fixed << setprecision(0) << Global::player->m_nrgyCurrent;
    string strNRGYCurrent = streamNRGYCurrent.str();

    stringstream streamNRGYMax;
    streamNRGYMax << std::fixed << setprecision(0) << Global::player->m_nrgyMax;
    ...
    int nrgyBarFilledWidth = barWidth * (double)Global::player->m_nrgyCurrent / Global::player->m_nrgyMax;
